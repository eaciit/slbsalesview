viewModel.appName = ko.observable('SLB Sales View')
viewModel.title = ko.observable('')
viewModel.title.subscribe(function (value) {
    var text = (value != '' ? (value + ' | ') : '') + viewModel.appName()
...
viewModel.ajaxPost = function (url, data, callbackOK, callbackNope) {
    ...
    if (typeof callbackOK === 'undefined') callbackOK = $.noop
    if (typeof callbackNope === 'undefined') callbackNope = $.noop
...
viewModel.getNavigationMenu = function () {
    viewModel.ajaxPostCallback('/access/getaccessmenuforcurrentuser', {}, function (data) {
        if ((data || []).length == 0) {
            return
        }
...
viewModel.newPromise = function () {
    return new Promise(function (resolve, reject) {
        return resolve()
    })
}
window.newPromise = viewModel.newPromise
